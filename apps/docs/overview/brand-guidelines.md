...
<p align="center"><em>Non-custodial USDC yield, routed to whichever protocol is paying more, for savers who need their money to hold its value first.</em></p>

<p align="center"><small>v1.0 — 2026</small></p>
...
**Stability** — Dollar-denominated, stablecoin-based. The goal is capital that holds its value first, and grows second.
...
**Audience** — Emerging-market savers, West Africa first — people whose local currency has already taught them not to trust promises about their money.

**Voice** — Direct, calm, specific. States what happens and why. Never hypes a rate, never hides material risk or the current state of the deployment.

## Logo & concept: Convergence

Three arcs share one center — Blend, DeFindex, and room for whatever routes are added later — each ending flush on the same line instead of trailing off. The landing dot is where a deposit actually sits: not a fixed point, but wherever the outermost arc currently resolves. One continuous idea, not a letterform or a borrowed icon.

The construction is four concentric radii (r14, r11, r8, r5) from one center on a 40×40 unit grid; the fourth (r5) is the landing dot.

**Clear space** — Minimum clear space on every side equals the radius of the landing dot. Nothing else (text, edges, other marks) enters that margin.
...
<table>
<tr>
<td align="center" style="background:#070d19;padding:24px;border-radius:8px">
...
<td align="center" style="background:#f4f6fb;padding:24px;border-radius:8px">
...
<td align="center" style="background:#10b981;padding:24px;border-radius:8px">
...
<td align="center" style="background:#f0f4ff;padding:24px;border-radius:8px">
  <svg viewBox="4 4 32 17" width="72" fill="none"><path d="M34 20 A14 14 0 0 0 6 20 A11 11 0 0 1 28 20 A8 8 0 0 0 12 20 A5 5 0 0 1 22 20" stroke="#070d19" stroke-width="2.25"/><circle cx="17" cy="17.3" r="2.6" fill="#070d19"/></svg><br/>
  <span style="color:#070d19;font-size:13px">Monochrome ink</span>
</td>
</tr>
</table>

**Small sizes / favicon** — the gradient drops to solid `#10B981` (Settled) below 24px so it never bands. Below 24px, icon only; never pair the mark this small with the wordmark. See `apps/web/public/brand/` for the source SVGs (`logo-mark.svg`, `logo-mark-solid.svg`) and the rendered favicons at 16, 32, 180, and 512px.
...
- **Don't recolor it.** Blue-to-green is the story: capital entering, capital settled.
- **Don't stretch or squash it.** Scale uniformly, always.
- **Don't rotate it.** The baseline is fixed; it's a horizon, not a wheel.
...
<tr><td style="background:linear-gradient(90deg,#3b82f6,#10b981);border-radius:6px">&nbsp;</td><td><strong>Convergence gradient</strong> — Entry → Settled, mark only</td></tr>
</table>

**Usage** — Night is the default surface; the product is dark by default, light is the alternate, not the other way round. The gradient belongs to the mark alone: never apply it to text, buttons, or UI chrome.

**Contrast** — Paper on Night: 17.7:1. Entry on Night: 5.3:1. Settled on Night: 7.7:1. All pass WCAG AA for text at any size used in product.

## Typography

One family, no exceptions: **Inter**, everywhere. It's already the product's system font; the mark carries the personality, the type stays out of the way and stays legible on low-end Android screens.
...
- **App icon** — the mark on a Deep (`#0D1E35`) tile with rounded corners.
- **Browser tab / favicon** — the solid-green mark below 24px, the gradient mark at 32px and above, on a Deep tile.
- **Product header** — the gradient mark at small scale beside the "Meridian" wordmark.
- **Social avatar** — the mark inverted (dark stroke on a Settled-green circle).
