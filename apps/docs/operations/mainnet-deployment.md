...
| ------------------------------------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migration timelock (`MIN_LEDGER_GAP`)                                           | 17,280 ledgers (~1 day)          | `packages/contracts/vault/src/storage.rs`                               | Raised from ~1 minute in [#557](https://github.com/drydocs/meridian/issues/557) specifically so this is a real observation window on mainnet, not just a valuation-stability check. Do not lower this for a "faster" mainnet migration flow without understanding you are removing the thing [#557](https://github.com/drydocs/meridian/issues/557) exists for. |
...
| Instance TTL bump / threshold (`INSTANCE_BUMP` / `INSTANCE_THRESHOLD`)          | 30 days / 29 days                | `packages/contracts/adapter-common/src/lib.rs`                          | Extended on every state-changing vault/adapter call ([#553](https://github.com/drydocs/meridian/issues/553)/#704) so a contract cannot archive itself between active use. Comfortably covers the 1-day migration timelock above; if that timelock is ever lengthened further, re-check it still fits well inside this window.                                   |
...
| DeFindex adapter slippage floor (`SLIPPAGE_BPS`)                                | 10 bps (0.1%)                    | `packages/contracts/defindex-adapter/src/lib.rs`                        | Floors `amounts_min`/`min_amounts_out` on DeFindex deposit/withdraw so those calls reject execution materially below what was requested ([#558](https://github.com/drydocs/meridian/issues/558)), rather than accepting any price DeFindex happens to offer.                                                                                                    |
...
`begin_migration`'s ~1-day timelock (see parameter selection above) is the intended detection-and-reaction window for a malicious `begin_migration` call, and the [alert keeper](./alert-keeper.md) is what surfaces that immediately rather than by chance; what to actually do about it once surfaced is [Incident Response](./incident-response.md)'s territory. `get_migration_snapshot()` and the admin-history endpoints (`getRpcAdminHistory`) are readable by anyone, not just the admin, and are useful for independently verifying on-chain state during any of the above rather than trusting the frontend's cached view of it.

## Go-live checklist

The security-relevant issues this runbook's proposal cross-references are all resolved as of this writing:

- [x] [#557](https://github.com/drydocs/meridian/issues/557): admin migration slippage cap and timelock (this page's parameter-selection table)
- [x] [#551](https://github.com/drydocs/meridian/issues/551): `MeridianVault::initialize()` front-running (constructor-argument pattern, see testnet doc)
- [x] [#564](https://github.com/drydocs/meridian/issues/564): mUSDC supply exclusivity (custom SEP-41 token, [#578](https://github.com/drydocs/meridian/issues/578))
- [x] [#558](https://github.com/drydocs/meridian/issues/558): zero slippage floor on DeFindex adapter deposit/withdraw
- [x] [#570](https://github.com/drydocs/meridian/issues/570): overflow error on divide-by-zero
...
- [ ] `ADMIN` key custody finalized (hardware-backed or multisig, per "Prerequisites specific to mainnet") and its holder(s) briefed on the [incident-response runbook](./incident-response.md)
- [ ] Configure `MERIDIAN_ALERT_WEBHOOK_URL` (unset as of the mainnet launch) so admin-action alerts actually reach someone, per [Incident Response](./incident-response.md#detection-what-s-actually-watching-right-now)
...
- [x] [Incident-response runbook](./incident-response.md) landed; still needs its holder(s) actually briefed on it, and `MERIDIAN_ALERT_WEBHOOK_URL` configured (see the two items above) before a live incident has both a playbook and something that surfaces one in real time
- [x] `MERIDIAN_KEEPER_SECRET_KEY` funded (`mainnet-accrual-keeper`, `GDJGFHB4PFTYBSQ3DDANCK5ANUT4OD373GRIFPELMGAZAU4USANEX7RC`) and configured on the production deployment; `accrue()` has no auth requirement, so no custody decision was needed for this one.
- [ ] `MERIDIAN_MIGRATION_KEEPER_SECRET_KEY` funded and its custody model decided. `mainnet-migration-keeper` (`GCZTV2UJ223WVGMPHTFEHCIJRO7HPF4MTS74X5GD5DH7MYNFH235BPRU`) is generated but not wired up: `migrate_adapter()` is admin-gated with no separate operator role, so this key can only get that authority via a full `transfer_admin` (rejected: hands total vault control to an automated key sitting in a Vercel env var) or by converting `ADMIN` to a multisig first with this key meeting only the "medium" threshold. Blocked on the `ADMIN` custody item above; deliberately deferred, not overlooked.
- [x] `UPSTASH_REDIS_REST_URL`/`_TOKEN` and `CRON_SECRET` configured on the production deployment (Upstash via Vercel's marketplace-integration naming, `UPSTASH_REDIS_REST_KV_REST_API_URL`/`_TOKEN`, which `keeper-state.ts` already falls back to)
- [ ] `MERIDIAN_ALERT_WEBHOOK_URL` configured before the alert keeper does anything beyond its current clean no-op (see `isAlertKeeperConfigured`, PR [#735](https://github.com/drydocs/meridian/issues/735)). Venue decided (Discord), not yet set up.
